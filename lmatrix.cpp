...
void lmatrix::solve(int t[], char original_hidden) {
	...
	INFO = LAPACKE_dggev(LAPACK_COL_MAJOR, 'N', 'V', d*m, l0->data(), LDA, l1->data(), LDB, alphar, alphai, beta, 0, LDV, v.data(), LDV);
...
//			if(abs(temp) >= LIMIT) {  /*hidden.real not close to 0 */
			if(abs(lambda(i,0)) >= LIMIT && abs(lambda(i,1)) <= LIMIT) {  /*hidden.real not close to 0 and hidden.imagine close to 0*/
				...
				if(pos<0) {	                            //this is a new solution
					solutions[bottom][0] = sign*temp;        //insert to solutions
					pos = bottom;                       //temporarily save bottom
					bottom++;	                    //update index of last solution
				}
				solutions[pos][1] = i;			//store index in lambda
	...
	for(int j=0; j<d*m; j++) {
		if(solutions[j][2] == 1) {
			int i= solutions[j][1];	//retrieve index in lambda
			if(abs(v(lim-1,i)) >= LIMIT && abs(v(lim-2,i)) >= LIMIT)
				solutions[j][1] = v(lim-2,i)/v(lim-1,i);	/*insert solution-not-hidden */
			else
				solutions[j][2] = 0;
		}
	}
